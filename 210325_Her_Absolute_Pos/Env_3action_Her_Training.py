...
class Gazebo_Env():
    def __init__(self, n_agents, n_targets, n_laser):
        ...
        # Her
        self.goal_pos = [0, 0] #Original Goal Pos
        self.goal_state = np.zeros((2, 2)) #agent's goal pos
    ...
    def make_state(self):
        ...
        for i in range(self.n_agents):
            state_list[i] = ((self.pos_list[self.start_target_index] - self.map_size[0]/2)/self.map_size[0]/2).tolist()[:2]
    ...
    def reset(self):
        ...
        for i in range(self.n_agents):
            self.goal_state[i] = self.goal_pos
    ...
    def step(self, action):
        ...
        return self.state, self.laser_list, self.is_gameover(), self.compute_reward(self.state, self.goal_pos), None
    ...
    # Terminal Reward needed?
    def compute_reward(self, state, goal):
        eps = 0.2
        ret = np.zeros(2)
        for i in range(self.n_agents):
            dist = mag_2d_vec(state[i, :2], goal)
            if (dist < eps):
                ret[i] = 1
    ...
    def make_new_goal(self):
        ...
        new_goal = [random.random() * self.map_size[0], random.random() * self.map_size[0]]
