...
class Gazebo_Env():
    def __init__(self, n_agents, n_targets, n_laser):
        ...
        self.min_laser = -0.90 # low for test
    ...
    def step(self, action):
        ...
        if (self.target_manuver == 1):
            ...
            if (self.map == 1):
                ...
                min_point = (np.array(min_point) - self.inital_position[self.start_target_index][:2]).tolist()
                self.goal_list[self.start_target_index].data = [8.0] + min_point + [10, heading_angle] + [0.7, 0.7]
            ...
            for j in range(self.n_agents):
                if (mag_3d_vec(self.pos_err_jtoi_from_j_frame[i][j]) < self.min_distance):
                    ...
                    for k in range(self.total_drone):
                        self.goal_list[k].data = [3.0] + zero_initial_position_list[k] + [self.start_angle[k]] + [1.0, 1.0]
                        self.pub_goal_list[k].publish(self.goal_list[k])
                    return True
    ...
    def get_reward(self):
        ...
        if (self.reward_type == 1): # Sparse Reward
            ...
            # Safety Condition
            for i in range(self.n_agents):
                laser_reward_list[i] += min(self.laser_list[i])
