...
class MADDPG:
    ...
    def update_policy(self):
        ...
        for agent in range(self.n_agents):
            per_list = list(permutations([i for i in range(config.n_agents)]))
            ...
            for per in per_list:
                ...
                for b in range(len(h_in_list)):
                    for a in range(self.n_agents):
                        if (type(h_in_list[b][per[a]]) != torch.Tensor):
                            h_in_list[b][per[a]] = torch.stack(h_in_list[b][per[a]]).type(FloatTensor)
                ...
                for b in range(len(h_out_list)):
                    for a in range(self.n_agents):
                        if (type(h_out_list[b][per[a]]) != torch.Tensor):
                            h_out_list[b][per[a]] = torch.stack(h_out_list[b][per[a]]).type(FloatTensor)
                    h_out_list[b] = torch.stack(h_out_list[b]).type(FloatTensor)
                h_out_batch = torch.stack(h_out_list).type(FloatTensor)

                h_out_batch = h_out_batch.squeeze(4) # right h_out_batch : batch x n_agents x num_output x (num_layer * num_direction) x (dummy : 1)batch x hidden 
                h_out_batch = h_out_batch.transpose(0, 2) # right h_out_batch(torch type) : batch x n_agents x num_output x (num_layer * num_direction) x hidden 
                h_out_batch = h_out_batch.transpose(1, 3) # right h_out_batch(torch type) : num_output x n_agents x batch x (num_layer * num_direction) x hidden 
                
                # h_out_batch : num_output x (num_layer * num_direction) x batch x n_agnets x hidden
                h_out_batch_agents = h_out_batch.contiguous().view(2, 1, self.batch_size, -1)
                #######################################################
                ...
                next_action_batch = [self.actors_target[i](next_state_batch[:, i, :], next_laser_batch[:, i, :], (h_out_batch[0, :, :, i, :].contiguous(), h_out_batch[1, :, :, i, :].contiguous()))[0] for i in range(self.n_agents)]
                ...
                agent_c_loss += loss_Q.detach()
                agent_a_loss += actor_loss.detach()

            c_loss.append(agent_c_loss)
            a_loss.append(agent_a_loss)

        for i in range(self.n_agents):
            # for cnt in range(3): #check
...
##################################################################################
def main():
    env = Gazebo_Env(config.n_agents, config.n_targets, config.dim_laser, args.test)
    reward_record = []
    ...
    for i_episode in range(start_epi, config.n_episode):
        ...
        past_obs_list, past_laser_list, past_action_list, past_h_in_list, past_h_out_list = [], [], [], [], []
        ...
        while not done:
            ...
            # change hidden parameter to cpu
            ...
            #     s, l, a, sp, lp, r, h_in, h_out, d = past_obs_list.pop(0), past_laser_list.pop(0), past_action_list.pop(0), obs_, laser_, reward, past_h_in_list.pop(0), past_h_out_list.pop(0), done
            #     maddpg.memory.push(s, l, a, sp, lp, r, h_in, h_out, d)
            ...
            #     new_lp = lp[per, :]
            #     new_r = r[per]
            #     new_h_in = [h_in[1], h_in[0]]
            #     new_h_out = [h_out[1], h_out[0]]
            #     maddpg.memory.push(new_s, new_l, new_a, new_sp, new_lp, new_r, new_h_in, new_h_out, d)
            ...
            if (n_step >= config.delay_step):
                maddpg.memory.push(past_obs_list.pop(0), 
                                    ...
                                    laser_, 
                                    reward, 
                                    past_h_in_list.pop(0), 
                                    past_h_out_list.pop(0), 
                                    done)
            ...
            #     sleep_dur = 0.1 - (cur_time - prev_time)
            #     if (sleep_dur > 0):
        ...
        if (args.test == False and c_loss != None):
            ...
            writer.add_scalar("Agent0 Critic Loss", c_loss[0], i_episode)
            writer.add_scalar("Agent1 Critic Loss", c_loss[1], i_episode)

            writer.add_scalar("Agent0 Actor Loss", a_loss[0], i_episode)
            writer.add_scalar("Agent1 Actor Loss", a_loss[1], i_episode)
