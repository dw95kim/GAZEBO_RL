...
class Gazebo_Env():
    def __init__(self, n_agents, n_targets, n_laser, is_test):
    ...
    def step(self, action):
        ...
        #     self.file = open('delay_txt/yaw_delay.txt', "w")
        
        # # data = str(self.vel_err_jtoi_from_j_frame[0][0][0]) + "\n"
        # data = str(self.angular_vel_list[0]) + "\n"
        ...
        for i in range(self.n_agents):
            ...
            if (i == 0):
                self.vel_cmd_list[i] = [0, 0, satmax(1.5*(self.inital_position[i][2] - self.pos_list[i][2]), 1) + 0.3*(0.0 - self.vel_list[i][2]), 1]
            else:
                self.vel_cmd_list[i] = [0, 0, satmax(1.5*(self.inital_position[i][2] - self.pos_list[i][2]), 1) + 0.3*(0.0 - self.vel_list[i][2]), 0]
            self.goal_list[i].data = [7.0] + self.vel_cmd_list[i].tolist() + [0.0, 0.0]
    ...
    def get_reward(self):
        ...
        if (self.reward_type == 1): # Sparse Reward
            ...
            velocity_reward_list = np.zeros(self.n_agents)
            yaw_reward_list = np.zeros(self.n_agents)
            terminal_reward_list = np.zeros(self.n_agents)
            laser_reward_list = -np.ones(self.n_agents)
