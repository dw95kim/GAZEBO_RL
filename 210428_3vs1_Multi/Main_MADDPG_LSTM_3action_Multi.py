...
import Utils
from Env_3action_Multi import Gazebo_Env
...
class MADDPG:
    def __init__(self, 
        ...
        if(args.test == False):
            ...
            self.var = [0.3 for i in range(n_agents)]
        else:
    ...
    def update_policy(self):
        ...
        for agent in range(self.n_agents):
            ...
            temp = [i for i in range(self.n_agents)]
...
def main():
    ...
    for i_episode in range(start_epi, config.n_episode):
        ...
        while not done:
            ...
            store_h_in_list = [(h_in_list[i][0].data.cpu(), h_in_list[i][1].data.cpu()) for i in range(config.n_agents)]
            store_h_out_list = [(h_out_list[i][0].data.cpu(), h_out_list[i][1].data.cpu()) for i in range(config.n_agents)]
            ...
            # per_list = permutations([i for i in range(config.n_agents)])
            ...
            #         new_lp = lp[per, :]
            #         new_r = r[per]
            #         new_h_in = [h_in[i.item()] for i in per]
            #         new_h_out = [h_out[i.item()] for i in per]
