...
class MADDPG:
    ...
    def update_policy(self):
        ...
        for agent in range(self.n_agents):
            ...
            whole_h_in = (h_in_batch_agents[0], h_in_batch_agents[1])
            ...
            target_Q[non_final_mask] = self.critics_target[agent](
                ...
                (h_out_batch_agents[0], h_out_batch_agents[1])
            ).squeeze()
            ...
            action_i, _ = self.actors[agent](state_i, laser_i, (h_in_batch[0, :, :, agent, :].contiguous(), h_in_batch[1, :, :, agent, :].contiguous()))
...
class Critic(nn.Module):
    def __init__(self, n_agent, dim_observation, dim_action, laser_cnt, hidden_layer=128, second_hidden_layer=64):
        ...
        self.PreFC = nn.Linear(las_dim, 10 * n_agent)
        self.FC1 = nn.Linear(obs_dim + 10 * n_agent + act_dim, hidden_dim)
    ...
    for i_episode in range(start_epi, config.n_episode):
        ...
        while not done:
            ...
            if (n_step < config.delay_step):
                ...
                past_h_in_list.append(h_in_list)
                past_h_out_list.append(h_out_list)
            elif (done):
