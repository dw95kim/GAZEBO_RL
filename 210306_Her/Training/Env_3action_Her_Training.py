...
class Gazebo_Env():
    def __init__(self, n_agents, n_targets, n_laser):
        ...
        # For Her
        self.pos_err_ttog_i_frame = np.zeros((self.n_agents, 2))
        self.goal_state = np.zeros((self.n_agents, 2))
        self.goal_pos = [0, 0, 0]
        ...
        # Done Part
        self.capture_done = 0
        self.collision_done = [0, 0]
        self.out_field_done = [0, 0]
        self.is_detect = 0
        ...
        self.target_manuver = 3 # 1 : Circle , 2 : Avoid Prob, 3 : Potential
    ...
    def callback_pos(self, data, agents):
        ...
        if (agents < self.start_target_index):
            ...
            self.pos_err_ttog_i_frame[agents] = np.matmul(rot_matrix, self.goal_pos - self.pos_list[self.start_target_index])[:2]
            goal_distance = mag_2d_vec(self.pos_err_ttog_i_frame[agents])/self.map_size[0]
            goal_yaw = math.atan2(self.pos_err_ttog_i_frame[agents][1], self.pos_err_ttog_i_frame[agents][0])
            self.goal_state[0] = goal_distance
            self.goal_state[1] = goal_yaw
    ...
    def reset(self):
        ...
        self.goal_pos = self.make_new_goal()
    ...
    def step(self, action):
        ...
        return self.state, self.laser_list, self.is_gameover(), self.compute_reward(self.state, self.goal_state), None
    ...
    def compute_reward(self, state, goal):
        # state : n_agent x n_state  : (2 x 12)
        # goal  : n_agent x goal     : (2 x 2)
        # reward = -np.ones(self.n_agents)
        reward = -np.zeros(self.n_agents)
        # dis_err = 0.05 # 0 ~ 1 --> 2m (map size : 40)
        # ang_err = 0.17 # rad --> 10deg
        for i in range(len(reward)):
            a = state[i][0]
            b = goal[i][0]
            ang = abs(state[i][1] - goal[i][1])
            reward[i] = a ** 2 + b ** 2 - 2 * a * b * math.cos(ang)
            # if (abs(state[i][0] - goal[i][0]) < dis_err and abs(state[i][1] - goal[i][1]) < ang_err):
            #     reward[i] = 0
    ...
    def make_new_goal(self):
        ...
        new_goal = [random.random() * self.map_size[0], random.random() * self.map_size[0], 10]

        while (inobstaclelist(new_goal, obs_pos_list, obs_wh_list)):
            new_goal = [random.random() * self.map_size[0], random.random() * self.map_size[0], 0.0]
