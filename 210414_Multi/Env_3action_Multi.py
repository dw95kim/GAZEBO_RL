...
class Gazebo_Env():
    def __init__(self, n_agents, n_targets, n_laser):
        ...
        self.shortest_fri_idx = -np.ones(self.n_agents)
        ...
        self.vel_cmd_list = np.zeros((self.n_agents, 4))
        ...
        self.start_angle = [0.0 for i in range(self.total_drone)]
        self.time_constrain = 1500.0 * self.n_agents
        self.min_distance = 1.5
        self.agent_fov = 120 # drgree
        self.target_manuver = 0 # 1 : Circle , 2 : Avoid Prob, 3 : Potential
    ...
    def callback_pos(self, data, agents):
        ...
        if (agents == 5):
            self.pos_list[agents] = np.array([data.pose.position.x, data.pose.position.y, data.pose.position.z]) + np.array([self.inital_position[agents][0], self.inital_position[agents][1], -3])
        else:
    ...
    def make_state(self):
        ...
        for i in range(self.n_agents):
            distance_list = []
            for j in range(self.start_target_index, self.total_drone):
                distance_list.append(mag_2d_vec(self.pos_err_jtoi_from_j_frame[self.start_target_index][i][:2])/self.map_size[0])
            
            distance_target = min(distance_list)
            min_idx = distance_list.index(distance_target)
            yaw_target = math.atan2(self.pos_err_jtoi_from_j_frame[self.start_target_index + min_idx][i][1], self.pos_err_jtoi_from_j_frame[self.start_target_index + min_idx][i][0])
            ...
            distance_list, yaw_list = [], []
            min_index = -1
            for j in range(self.n_agents):
                if j != i:
                    distance_list.append(mag_2d_vec(self.pos_err_jtoi_from_j_frame[j][i][:2])/self.map_size[0])
                else:
                    distance_list.append(999) # big number
            
            distance_friends = min(distance_list)
            min_idx = distance_list.index(distance_friends)
            self.shortest_fri_idx[i] = min_idx
            yaw_friends = math.atan2(self.pos_err_jtoi_from_j_frame[min_idx][i][1], self.pos_err_jtoi_from_j_frame[min_idx][i][0])
    ...
    def reset(self):
        ...
        for i in range(self.total_drone):
            ...
            if (i == 5):
                zero_initial_position_list.append([temp_position[0], temp_position[1], 13])
            else:
    ...
    def step(self, action):
        ...
        if (self.target_manuver == 1):
            ...
            if (self.map == 1):
                ...
                tracking_drone_pos = [self.pos_list[i][:2] for i in range(self.n_agents)]
                eps = 0.000001

                masked = np.zeros((self.n_targets, self.n_agents))
                for t in range(self.n_targets):
                    for j in range(self.total_drone):
                        if (t + self.start_target_index == j):
                            masked[t][j] = 1
                        else:
                            for i in range(len(obs_pos_list)):
                                obs_pos = obs_pos_list[i]
                                obs_wh = obs_wh_list[i]
                                if (isboxmasked(self.pos_list[j][:2], self.pos_list[t + self.start_target_index][:2], obs_pos, obs_wh)):
                                    masked[t][j] = 1
                                    break
                    ...
                    for i in range(self.map_size[0] + 2):
                        for j in range(self.map_size[0] + 2):
                            ...
                            drone_force = 0
                            for k in range(self.total_drone):
                                if (masked[t][k] == 0):
                    ...
                    cur_pix = np.array([int(round(self.pos_list[t + self.start_target_index][0])), int(round(self.pos_list[t + self.start_target_index][1]))])
                    if (cur_pix[0] == 0): cur_pix[0] += 1
                    if (cur_pix[0] == self.map_size[0] + 1): cur_pix[0] -= 1
                    if (cur_pix[1] == 0): cur_pix[1] += 1
                    if (cur_pix[1] == self.map_size[0] + 1): cur_pix[1] -= 1
                    ...
                    heading_vector = (np.array(min_point) - self.pos_list[t + self.start_target_index][:2])
                    heading_angle = math.atan(heading_vector[1]/heading_vector[0])

                    min_point = (np.array(min_point) - self.inital_position[t + self.start_target_index][:2]).tolist()
                    self.goal_list[t + self.start_target_index].data = [8.0] + min_point + [10, heading_angle] + [1.0, 1.0]
    ...
    def is_gameover(self):
        if (self.map == 2):
            ...
            for i in range(self.total_drone):
                ...
                if (len(drone_pos_list) == 0):
                    while(inobstaclelist(temp_position[:2], obs_pos_list, obs_wh_list)):
                        temp_position = [random.random() * self.map_size[0], random.random() * self.map_size[0], 0.0]
                    drone_pos_list.append(temp_position)    
                else:
                    dist_temp_pos_list = []
                    for j in range(len(drone_pos_list)):
                        dist_temp_pos_list.append(distance_2d_vec(temp_position[:2], drone_pos_list[j][:2]))
                    
                    while(inobstaclelist(temp_position[:2], obs_pos_list, obs_wh_list) or min(dist_temp_pos_list) < self.min_distance):
                        temp_position = [random.random() * self.map_size[0], random.random() * self.map_size[0], 0.0]
            ...
            for i in range(self.total_drone):
                self.offset_list[i] = np.array(drone_pos_list[i])
        ...
        if (self.n_targets > 1):
            if (mag_3d_vec(self.pos_err_jtoi_from_j_frame[self.start_target_index][self.start_target_index + 1]) < self.min_distance):
                self.target_collision_done = 1
                print("Collision Target Drones")
    ...
    def get_reward(self):
        ...
        for i in range(self.n_agents):
            target_distance_list.append(distance_2d_vec(self.pos_list[self.start_target_index][:2], self.pos_list[i][:2]))

        if (self.reward_type == 1): # Sparse Reward
            terminal_reward_list = np.zeros(self.n_agents)
            for i in range(self.n_agents):
                # Terminal Condition
                if (self.out_field_done[i] == 1):
                    terminal_reward_list[i] -= -1

                if (self.collision_done[i] == 1):
                    terminal_reward_list[i] -= -1
                
                if (self.capture_done[i] == 1):
                    terminal_reward_list[i] += 2

            reward = terminal_reward_list - 0.01
            ...
            for i in range(self.n_agents):
                if (self.capture_done[i] == 1):
                    reward[i] = max(5 * (self.time_constrain - self.start_time) / self.time_constrain, 1)

            if (self.target_collision_done == 1):
                for i in range(self.n_agents):
                    reward[i] = 1
