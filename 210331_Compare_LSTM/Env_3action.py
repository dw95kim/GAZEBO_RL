...
class Gazebo_Env():
    ...
    def get_reward(self):
        ...
        if (self.reward_type == 1): # Sparse Reward
            ...
            # Distance
            # if (sum(self.prev_target_distance) != 0):
            #     for i in range(self.n_agents):
            #         target_distance = target_distance_list[i]
            #         prev_target_distance = self.prev_target_distance[i]
            #         distance_reward_list[i] = prev_target_distance - target_distance
            for i in range(self.n_agents):
                target_distance = target_distance_list[i]
            ...
            # reward = (1/2500.0) * distance_reward_list + terminal_reward_list
            # reward = (1/2500.0) * distance_reward_list + terminal_reward_list + (1/1000.0) * laser_reward_list
            reward = (1/250.0) * distance_reward_list
            # print(distance_reward_list, laser_reward_list)

        # for first time
        if (sum(self.prev_target_distance) == 0): 
            for i in range(self.n_agents):
                self.prev_target_distance[i] = target_distance_list[i]
